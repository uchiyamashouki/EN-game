...
export class BattleScene {
  ...
  render() {
    ...
    this.root.innerHTML = `
      <div class="panel battle">
        ...
        <div class="arena">
          ...
          <div class="actor enemy" id="enemy-art">${enemy.art}</div>
        </div>
    ...
    this.root.querySelectorAll("[data-cmd]").forEach((btn) => {
      btn.addEventListener("click", () => this.playerTurn(btn.dataset.cmd, false));
    });
  ...
  createEnemy(isBoss) {
    const list = isBoss
      ...
      : [{ name: "スライム", hp: 55, art: "🟢" }, { name: "ゴブリン", hp: 65, art: "👹" }, { name: "バット", hp: 50, art: "🦇" }];
    const pick = list[Math.floor(Math.random() * list.length)];
    return { name: pick.name, maxHp: pick.hp + (this.state.stage - 1) * 5, art: pick.art, isBoss };
  }

  async playerTurn(command, isBossOverride) {
    if (this.state.playerHp <= 0 || this.state.enemyHp <= 0) return;
    const isBoss = isBossOverride ?? this.enemy?.isBoss;
    const questions = selectQuestionSet({ command, stage: this.state.stage, state: this.state, isBoss });
